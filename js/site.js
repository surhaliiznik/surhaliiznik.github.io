...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (inputField) {
        ...
        const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    { 
                        role: "system", 
                        content: "Sen İznik Sur Halı'nın samimi ve uzman satış danışmanısın. Müşterilere evleri için en uygun halı, kaymaz yolluk, makine halısı ve özel ölçü kesim seçeneklerinde yardımcı olursun. Kısa, açık ve samimi cevaplar ver." + categoryInstruction + productsInstruction 
                    },
                    { role: "user", content: userMessage }
                ]
            })
        ...
        const data = await response.json();
        return data?.choices?.[0]?.message?.content?.trim() || "Üzgünüm, şu anda yanıt oluşturamıyorum.";

    } catch (error) {
        console.error("Groq AI yanıt hatası:", error);
        return "Şu anda asistan servisine ulaşılamıyor. Lütfen daha sonra tekrar deneyiniz veya WhatsApp üzerinden bizimle iletişime geçiniz.";
    }
}

// Sayfa yüklendiğinde tüm modülleri başlatan ana dinleyici
